...
def test_renew_endpoint_selects_renewable_current_loans(monkeypatch):
    monkeypatch.setattr(circulation.circulation_service, "get_borrower_current_loans", lambda **kwargs: [{"item_id": "I1", "can_renew": True}, {"item_id": "I2", "can_renew": False}])
    monkeypatch.setattr(circulation.circulation_service, "renew_items", lambda **kwargs: kwargs)
...
def test_create_hold_forwards_data(monkeypatch):
    monkeypatch.setattr(holds.hold_service, "create_hold", lambda **kwargs: kwargs)
...
def test_get_hold_forwards_id(monkeypatch):
    monkeypatch.setattr(holds.hold_service, "get_hold", lambda db, hold_id: {"id": hold_id})
    assert holds.get_hold(3, db="db") == {"id": 3}


def test_get_holds_for_borrower_forwards_flag(monkeypatch):
    monkeypatch.setattr(holds.hold_service, "get_holds_for_borrower", lambda *args, **kwargs: kwargs)
...
def test_get_holds_for_title_forwards_active_flag(monkeypatch):
    monkeypatch.setattr(holds.hold_service, "get_holds_for_bibliographic_record", lambda *args, **kwargs: kwargs)
...
def test_ready_holds_delegates(monkeypatch):
    expected = [{"id": 1}]
    monkeypatch.setattr(holds.hold_service, "get_ready_holds", lambda db: expected)
