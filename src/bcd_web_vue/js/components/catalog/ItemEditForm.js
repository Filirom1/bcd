...
export default {
  ...
  setup(props, { emit }) {
    ...
    // Keep track of the value generated by the shelf picker. If the user
    // edits the call number afterwards, a subsequent shelf change must not
    // silently overwrite that manual value.
    let lastGeneratedCallNumber = null;
    const updateCallNumberForShelf = (shelf) => {
      if (!shelf || shelf === '__clear__') return;
      const manuallyEdited = lastGeneratedCallNumber !== null
        && formData.value.call_number !== lastGeneratedCallNumber;
      const value = generateCallNumber(shelf);
      if (value && !manuallyEdited) {
        formData.value.call_number = value;
        lastGeneratedCallNumber = value;
      }
    };
    ...
    watch(() => formData.value.shelf_location, updateCallNumberForShelf);

    const handleShelfLocationChange = (shelf) => {
      if (shelf === '__clear__') {
        formData.value.shelf_location = '';
        return;
      }
      formData.value.shelf_location = shelf;
      updateCallNumberForShelf(shelf);
    };
